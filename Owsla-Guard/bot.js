...
RoleGuard.on("message", async (message) => {
    ...
    if ((Config.BotOwner.includes(message.author.id)) && command == "!güvenli") {
        ...
        let sec = args[0];
        if (!sec) return message.reply(`
**Doğru Kullanım**: **!güvenli ${["full", "rol&kanal", "rol", "kanal", "ban&kick", "bot", "chat","grol"].map(x => `\`${x}\`
`).join(" - ")}**`);
...
RoleGuard.on("guildMemberUpdate", async (oldMember, newMember) => {
    if (oldMember.roles.cache.size != newMember.roles.cache.size) {
        ...
        return sendLog(`
Verilmesi yasak olan bir rol sağ tık ile verildi
Veren kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Verilen kişi: **${newMember.user.tag}** (${newMember.id}) 
İşlemi gerçekleştiren kişiyi sunucudan yasakladım
`);
    }
});
...
RoleGuard.on("roleCreate", async (role) => {
    ...
    if (roleCreateLimit[entry.executor.id] && roleCreateLimit[entry.executor.id] >= Config.Limit.RoleCreate) {
        ...
        return sendLog(`Rol açma işleminde limit aşımına uğrandı
Rolu açan kişi **${entry.executor.tag}** (${entry.executor.id}) 
${Config.Limit.RoleCreate} sayısından fazla rol açmayı denediği için jaile attım`);
    };
    roleCreateLimit[entry.executor.id] += 1;
    setTimeout(() => {
        roleCreateLimit[entry.executor.id] = 0;
    }, 1000 * 60 * 3);
    sendLog(`Bir rol açma işlemi gerçekleşti
Rolu açan kişi **${entry.executor.tag}** (${entry.executor.id})
Kullanıcının geriye kalan rol açma limiti: **${roleCreateLimit[entry.executor.id]}/${Config.Limit.RoleCreate}**`)
});
...
RoleGuard.on("roleDelete", async (role) => {
    ...
    if ((!entry || await checkPermission(RoleGuard, entry.executor.id, "full")) && !veri.SafeRole.includes(role.id)) {
        ...
        return sendLog(`Rol silme işlemi gerçekleşti
Rolu silen kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Silinen rol: **${role.name}** (${role.id}) 
Silen kişi botun güvenli listesinde bulunduğu için hiçbir işlem yapmadım
`);
    }
    ...
    if (!result) {
        return sendLog(`Rol silme işlemi gerçekleşti
Rolü silen kişi: **${entry.executor.tag}** (${entry.executor.id})
Silinen Rol **${role.name}** (${role.id})
Rolü silen kişiyi sunucudan yasakladım, ancak silinen rol için bir veri olmadığı için hiçbir şey yapmadım.
`);
    }
    ...
    sendLog(`Rol silme işlemi gerçekleşti
Rolü silen kişi: **(${memb.displayName})** **${entry.executor.tag}** (${entry.executor.id}) 
Silinen Rol**${role.name}** (${role.id}
Role sahip **${result.members.length}** üye **${clients.length}**'ı bot üye olmak üzere rolü destekçiler ile birlikte dağıtmaya başlıyorum
İşlemin biteceği tahmini süre: **${(membersCount>1000 ? parseInt((membersCount*(Config.Guard.GiveRoleDelay/1000)) / 60)+" dakika" : parseInt(membersCount*(Config.Guard.GiveRoleDelay/1000))+" saniye")}**`);
    clients.forEach((_guardClient, _index) => {
...
ChannelGuard.on("channelCreate", async (channel) => {
    ...
    if (channelCreateLimit[entry.executor.id] >= Config.Limit.ChannelCreate) {
        ...
        return sendLog(`Kanal oluşturma işlemi gerçekleşti
Kanalı oluşturan kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Kullanıcı belirtilen kanal açma limit aşımı yaptığı için cezalandırıldı.`)
    }
    channelCreateLimit[entry.executor.id] += 1;
    setTimeout(() => {
        channelCreateLimit[entry.executor.id] = 0;
    }, 1000 * 60 * 3);
    sendLog(`Kanal açma işlemi gerçekleşti
Kanalı oluşturan kişi: **${entry.executor.tag}** (${entry.executor.id}) 
 Kullanıcının geriye kalan kanal açma limiti: **${channelCreateLimit[entry.executor.id]}**)`)
});
...
OtherGuard.on("guildMemberAdd", async (member) => {
    ...
    return sendLog(`Bot ekleme işlemi gerçekleşti
Botu ekleyen kişi: **${entry.executor.tag}** (${entry.executor.id})
Eklenen Bot:**${member.user.tag}** (${member.id}) 
Botu ekleyen kişiyi sunucudan yasakladım.
`);
});
...
OtherGuard.on("guildBanAdd", async (guild, user) => {
    ...
    if (BanLimit[entry.executor.id] && BanLimit[entry.executor.id].Now + 1 > Config.Limit.Ban) {
        ...
        return sendLog(`Ban atma işlemi gerçekleşti
Ban atan kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Yasaklanan kişi: **${user.tag}** (${user.id})
Kullanıcı belirtilen ban atma limitini aştığı için yasakladığı kişi ile birlikte sunucudan yasaklandı.`);
...
Ban atan kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Yasaklanan kişi: **${user.tag}** (${user.id})
Kullanıcının geriye kalan ban atma limiti: (${1})`);
    } else {
        BanLimit[entry.executor.id].Now += 1;
        setTimeout(() => {
            BanLimit[entry.executor.id] = {
...
Ban atan kişi: **${entry.executor.tag}** (${entry.executor.id}) 
Yasaklanan kişi: **${user.tag}** (${user.id}) 
Geriye kalan ban limiti (${BanLimit[entry.executor.id].Now})`);
    }
});
...
OtherGuard.on("guildMemberRemove", async (member) => {
    ...
    if (KickLimit[entry.executor.id] && KickLimit[entry.executor.id].Now + 1 > Config.Limit.Kick) {
        ...
        return sendLog(`
        **${entry.executor.tag}** (${entry.executor.id}) kişisi **${member.user.tag}**(${member.id}) kişisini ATTI. Kick limitini geçtiği için kendisi de banlandı.`);
...
Config.Guards.forEach(async guard => {
    ...
    guardClient.on("ready", () => {
        Guards.user.setStatus("invisible");
